...
class LeaderboardService {
    static async getSportLeaderboard(sport) {
        try {
            ...
            const standardizedTests = await client_1.prisma.standardizedTest.findMany({
            ...
            const psychologicalAssessments = await client_1.prisma.psychologicalAssessment.findMany({
                where: { athleteId: { in: athleteIds } },
                select: {
                    athleteId: true,
            ...
            psychologicalAssessments.forEach((assessment) => {
    ...
    static calculateSportSpecificScore(testResults) {
